...
const useGameStore = create<GameState>((set, get) => ({
  ...
  undoMove: () => {
    ...
    set({
      board: newBoard,
      currentPlayer: moveHistory[moveHistory.length - movesToUndo]?.player === 'black' ? 'black' : 'white',
...
function checkWinner(board: CellState[][], row: number, col: number, player: Player): Player | 'draw' | null {
  const directions = [
    [1, 0],   // horizontal
    [0, 1],   // vertical
    [1, 1],   // diagonal
